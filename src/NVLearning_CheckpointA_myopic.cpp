//============================================================================
// Name        : NVLearning_CheckpointA_myopic.cpp
// Author      : Tong WANG
// Email       : tong.wang@nus.edu.sg
// Version     : v7.2 (2013-07-16)
// Copyright   : ...
// Description : general code for newsvendor with censored demand --- the Checkpoint-A (Inventory Checkpoint) myopic heuristic
//============================================================================
...
void archiveVMap(const boost::unordered_map<tuple<int, int, multiset<int>>, double> & vMap, const string& ofile)
{
...
void importVMap(boost::unordered_map<tuple<int, int, multiset<int>>, double> & vMap, const string& ifile)
{
...
void generateJobsToList(const int& n, const vector<tuple<int, int, multiset<int>>> & currentJobList, const boost::unordered_map<tuple<int, int, multiset<int>>, double> & currentVMap, vector<tuple<int, int, multiset<int>>> & nextJobList)
{
    set<tuple<int, int, multiset<int>>> nextJobSet;
    
    for (auto ii = currentJobList.begin(); ii != currentJobList.end(); ++ii)
    {
        int tt = get<0>(*ii);
        int qq = get<1>(*ii);
        multiset<int> co = get<2>(*ii);
        ...
        if (currentVMap.count(make_tuple(tt, qq, co))==0)
        {
            ...
            if (xx==0)
                //nextJobSet.emplace(tt, qq, co);
                nextJobSet.insert(make_tuple(tt, qq, co));
            else
            {
                
                //case 3
                multiset<int> co_new = co;
                co_new.insert(xx);
                //nextJobSet.emplace(tt, qq, co_new);
                nextJobSet.insert(make_tuple(tt, qq, co_new));
                
                //case 2
                for (int m=1; m<M; ++m)
                    for (int d_m=0; d_m<xx; ++d_m)
                    {
                        multiset<int> co_new = co;
                        co_new.insert(xx-d_m);
                        //nextJobSet.emplace(tt + m, qq + d_m, co_new);
                        nextJobSet.insert(make_tuple(tt + m, qq + d_m, co_new));
                    }
                
                //case 1
                for (int d=0; d<xx; ++d)
                    //nextJobSet.emplace(tt + M, qq + d, co);
                    nextJobSet.insert(make_tuple(tt + M, qq + d, co));
            }
		}
	}
...
void solveJobsInList(const int& n, const vector<tuple<int, int, multiset<int>>> & currentJobList, const boost::unordered_map<tuple<int, int, multiset<int>>, double> & nextVMap, boost::unordered_map<tuple<int, int, multiset<int>>, double> & currentVMap)
{
    ...
    for (int ii=0; ii<currentJobList.size(); ++ii)
    {
        ...
        multiset<int> co = get<2>(currentJobList[ii]);
        ...
        if (count==0)
        {
            ...
            else
            {
                ...
                vector< vector<double> > observation_pdf = observation_pdf_update(xx, tt, qq, co);
                ...
                out1 += ( price*xx + vv) * observation_pdf[0][xx-1];
                ...
                for (int m=1; m<M; ++m)
                    for (int d_m=0; d_m<xx; ++d_m)
                    {
                        if (n<N) {
                            multiset<int> co_new = co;
                            co_new.insert(xx-d_m);
                            vv = nextVMap.at(make_tuple(tt+m, qq+d_m, co_new));
                        }
                        
                        out1 += ( price*xx + vv) * observation_pdf[m][d_m];
                    }
                ...
                for (int d=0; d<xx; ++d)
                {
                    if (n<N) vv = nextVMap.at(make_tuple(tt+M, qq+d, co));
                    out1 += ( price*d + vv ) * observation_pdf[M][d];
                }
    ...
    //for (beta0=2; beta0>=0.05; beta0/=2)
    {
        ...
        //for (M=2; M<=M_MAX; M*=2)
        {
            ...
            for (cost=1.8; cost>=0.15; cost-=0.1)
            {
				...
				boost::unordered_map<tuple<int, int, multiset<int>>, double> vMap1, vMap2, vMap3, vMap4;
                vector<tuple<int, int, multiset<int>>> jobList1, jobList2, jobList3, jobList4;
